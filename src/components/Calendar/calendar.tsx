...
const Calendar = () => {
  ...
  return (
    ...
    //   </div>
    // </div>
    <div className="flex w-full ml-[20rem] divide-x-2 gap-10 items-center">
      <div className="w-96 h-96">
        <div className="flex justify-between items-center">
          <h1 className="select-none font-semibold">
            {months[today.month()]}, {today.year()}
        ...
        <div className="w-full grid grid-cols-7 text-gray-400">
          {days.map((day, index) => {
            return (
              <h1 key={index} className="h-14 place-content-center grid text-sm">
                {day}
              </h1>
            );
          })}
        ...
        <div className="w-full grid grid-cols-7">
          {generateDate().map(({ date, currentMonth, today }, index) => {
            return (
              <div key={index} className="h-14 place-content-center border-t grid text-sm">
                <h1
                  className={combine(
                    currentMonth ? '' : 'text-gray-400',
                    today ? 'bg-red-600 text-white' : '',
